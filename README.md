...
npx twcheck src --fix
npx twcheck src --fix-conflict
npx twcheck . --check
npx twcheck src --json
npx twcheck src --css src/styles/globals.css
```

The default mode is read-only. `--diff` previews exact replacements, `--fix` writes canonical replacements, and `--fix-conflict` removes the earlier class when two classes conflict on the same CSS property. Conflict fixes are opt-in because the intended visual value cannot be inferred safely. `--check` returns exit code `1` when canonical warnings or CSS conflicts are found. Runtime or configuration errors return exit code `2`.
...
`<path>` accepts a file, directory, or `.`. Available flags are `--diff`, `--fix`, `--fix-conflict`, `--check`, `--json`, `--css <file>`, `--help`, and `--version`.
